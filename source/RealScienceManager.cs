...
namespace RealScience
{
	...
	class RealScienceManager : MonoBehaviourWindowPlus
	{
		...
		private List<Part> onboardExperiments = new List<Part>();
		...
		protected override void DrawWindow(int id)
		{
			...
			foreach (Part part in onboardExperiments)
			{
				if (part == null)
					continue;

				foreach (RealScienceExperiment experiment in part.FindModulesImplementing<RealScienceExperiment>())
				{
					if (experiment == null)
						continue;
		...
		{
			onboardExperiments = FlightGlobals.ActiveVessel.Parts.Where(p => p.Modules.Contains("RealScienceExperiment")).ToList();
		}
	}
}
